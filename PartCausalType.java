...
/*   $Author:   zf297a  $
   $Revision:   1.2  $
       $Date:   02 Dec 2008 11:41:54  $
   $Workfile:   PartCausalType.java  $
        $Log:   I:\Program Files\Merant\vm\win32\bin\pds\archives\SDS-AMD\Components-ClientServer\WindowAlgorithm\PartCausalType.java.-arc  $
/*
/*   Rev 1.2   02 Dec 2008 11:41:54   zf297a
/*Removed s variable that was not being used.
/*
/*   Rev 1.1   02 Dec 2008 09:18:30   zf297a
/*Fixed comments and lterals for F1 (spo view) and F2 (amd x_ view)
/*
/*   Rev 1.0   12 Sep 2008 13:55:50   zf297a
/*Initial revision.

	*/

public class PartCausalType implements Rec {
    ...
    static String F2Instance = "amd_owner";
    static String F2InstanceSQL = "";

    static Logger logger = Logger.getLogger(PartCausalType.class.getName());
    ...
    private static TableSnapshot F1 = null ; // v_part_causal_type (spo)
    private static TableSnapshot F2 = null ; // x_part_causal_type_v (amd)

    // loads the parameter from this class's properties file if they exist
    static private void loadParams() {
	try {

		java.util.Properties p        = new AppProperties(PartCausalType.class.getName()).getProperties() ;
		...
		flatFileName = String.valueOf( p.getProperty("dumpFile", "PartCausalType.csv"));
		F1Instance = String.valueOf( p.getProperty("F1Instance", "SPOC17V2"));
		F2Instance = String.valueOf( p.getProperty("F2Instance", "amd_owner"));
	...
	if (useTestBatch) {
	    ...
	    batchNum = SpoBatch.createBatch("X_IMP_PART_CAUSAL_TYPE") ;			
	}
...
// keeps track of how long it took to read the 2 tables and form the arrays
	    ...
	    F1 = new TableSnapshot(bufSize, new PartCausalTypeFactory("SELECT * FROM " + F1InstanceSQL + 
	    "V_PART_CAUSAL_TYPE order by part", spo, prefetchSize)) ;
	    logger.debug("F1 created") ;
            F2 = new TableSnapshot(ageBufSize, new PartCausalTypeFactory("SELECT * FROM " + F2InstanceSQL + 
	    "x_part_causal_type_v order by part" , amd, prefetchSize)) ;
	...
	if (F1 != null) {
	   	System.out.println("v_part_causal_type=" + F1.getRecsIn()) ;
        	logger.info("v_part_causal_type=" + F1.getRecsIn()) ;
	}
	if (F2 != null) {
	        System.out.println("x_part_causal_type_v=" + F2.getRecsIn()) ;
        	logger.info("x_part_causal_type_v=" + F2.getRecsIn()) ;
	}
    ...
    class Body {
        ...
        double         quantity  ;
        public String toString() {
	 return "causal_type=" + causal_type
	 + " quantity=" + quantity ;
        ...
        public boolean equals(Object o) {
            Body b = (Body) o ;
            boolean result ;
            result = equal(b.causal_type, causal_type) ;
            showDiff(result, "causal_type", b.causal_type + "", causal_type + "") ;
    ...
    PartCausalType(ResultSet r) throws SQLException {
        try {
        key = new Key() ;
        key.part = r.getString("PART") ;

        body = new Body() ;
        body.causal_type = r.getString("CAUSAL_TYPE") ;
