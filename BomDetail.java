...
/*   $Author:   zf297a  $
   $Revision:   1.1  $
       $Date:   02 Dec 2008 11:17:22  $
   $Workfile:   BomDetail.java  $
        $Log:   I:\Program Files\Merant\vm\win32\bin\pds\archives\SDS-AMD\Components-ClientServer\WindowAlgorithm\BomDetail.java.-arc  $
/*
/*   Rev 1.1   02 Dec 2008 11:17:22   zf297a
...
public class BomDetail implements Rec {
    ...
    static Logger logger = Logger.getLogger(BomDetail.class.getName());
    ...
    private static TableSnapshot F1 = null ; // v_bom_detail
    private static TableSnapshot F2 = null ; // x_bom_detail_v

    // loads the parameter from this class's properties file if they exist
    static private void loadParams() {
	try {

		java.util.Properties p        = new AppProperties(BomDetail.class.getName()).getProperties() ;
		...
		flatFileName = String.valueOf( p.getProperty("dumpFile", "BomDetail.csv"));
	...
	if (useTestBatch) {
	    ...
	    batchNum = SpoBatch.createBatch("X_IMP_BOM_DETAIL") ;			
	}
...
// keeps track of how long it took to read the 2 tables and form the arrays
	    ...
	    F1 = new TableSnapshot(bufSize, new BomDetailFactory("SELECT * FROM " + F1InstanceSQL + 
	    "V_Bom_Detail order by bom, part,included_part", spo, prefetchSize)) ;
            F2 = new TableSnapshot(ageBufSize, new BomDetailFactory("SELECT * FROM " + F2InstanceSQL + 
	    "x_Bom_Detail_v order by bom, part, included_part" , amd, prefetchSize)) ;
	...
	if (F1 != null) {
	   	System.out.println("v_bom_detail=" + F1.getRecsIn()) ;
        	logger.info("v_bom_detail=" + F1.getRecsIn()) ;
	}
	if (F2 != null) {
	        System.out.println("x_bom_detail_v=" + F2.getRecsIn()) ;
        	logger.info("x_bom_detail_v=" + F2.getRecsIn()) ;
	}
    ...
    class Key implements Comparable {
        String      bom ;
        String      part ;
        String      included_part ;
        

        public boolean equals(Object o) {
            Key k = (BomDetail.Key) o ;
            return (k.bom.equals(bom) && k.part.equals(part) && k.included_part.equals(included_part) );
        }
        public int hashCode() {
            return bom.hashCode() + part.hashCode() + included_part.hashCode() ;
        }
        public int compareTo(Object o) {
            Key theKey ;
            theKey = (Key) o ;
            return (theKey.bom + theKey.part + theKey.included_part).compareTo(bom + part + included_part) ;
        }
        public String toString() {
          return "bom=" + bom + " part=" + part + " included_part=" + included_part  ;
        }
    }
    Key key ;
    Key keyToCompare;
    class Body {
	    ...
	    if (!result) { return result ; } ;
            result = result && equal(b.begin_date, b.begin_date_is_null, begin_date, begin_date_is_null) ;
            showDiff(result, "begin_date=", b.begin_date + "", begin_date + "") ;
	    if (!result) { return result ; } ;
    ...
    BomDetail(ResultSet r) throws SQLException {
        try {
        key = new Key() ;
        key.bom = r.getString("BOM") ;
        key.part = r.getString("PART") ;
        key.included_part = r.getString("INCLUDED_PART") ;
