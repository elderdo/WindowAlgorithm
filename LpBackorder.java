...
/*   $Author:   zf297a  $
   $Revision:   1.2  $
       $Date:   02 Dec 2008 11:29:56  $
   $Workfile:   LpBackorder.java  $
        $Log:   I:\Program Files\Merant\vm\win32\bin\pds\archives\SDS-AMD\Components-ClientServer\WindowAlgorithm\LpBackorder.java.-arc  $
/*
/*   Rev 1.2   02 Dec 2008 11:29:56   zf297a
...
/*
/*   Rev 1.0   13 May 2008 21:42:44   zf297a
/*Initial revision.

*/

public class LpBackorder implements Rec {
    ...
    static Logger logger = Logger.getLogger(LpBackorder.class.getName());
    ...
    private static TableSnapshot F1 = null ; // v_lp_backorder (spo)
    private static TableSnapshot F2 = null ; // x_lp_backorder_v (amd)

    // loads the parameter from this class's properties file if they exist
    static private void loadParams() {
	try {

		java.util.Properties p        = new AppProperties(LpBackorder.class.getName()).getProperties() ;
		...
		flatFileName = String.valueOf( p.getProperty("dumpFile", "LpBackorder.csv"));
	...
	if (useTestBatch) {
	    ...
	    batchNum = SpoBatch.createBatch("X_IMP_LP_BACKORDER") ;			
	}
...
// keeps track of how long it took to read the 2 tables and form the arrays
	    ...
	    F1 = new TableSnapshot(bufSize, new LpBackorderFactory("SELECT * FROM " + F1InstanceSQL + 
	    "V_lp_backorder order by location, part, backorder_type", spo, prefetchSize)) ;
	    if (F1 == null) {
		    System.out.println("F1: the old master was not set") ;
		    System.out.println("SELECT * FROM " + F1InstanceSQL + 
			    "V_lp_backorder order by location, part, backorder_type") ;
		    System.exit(4) ;
	    }
            F2 = new TableSnapshot(ageBufSize, new LpBackorderFactory("SELECT * FROM " + F2InstanceSQL + 
	    "x_lp_backorder_v order by location, part, backorder_type" , amd, prefetchSize)) ;
	    if (F2 == null) {
		    System.out.println("F2: the new master was not set") ;
		    System.out.println("SELECT * FROM " + F2InstanceSQL + 
	    "x_lp_backorder_v order by location, part, backorder_type" ) ;
		    System.exit(4) ;
	    }
	...
	if (F1 != null) {
	   	System.out.println("v_lp_backorder=" + F1.getRecsIn()) ;
        	logger.info("v_lp_backorder=" + F1.getRecsIn()) ;
	}
	if (F2 != null) {
	        System.out.println("x_lp_backorder_v=" + F2.getRecsIn()) ;
        	logger.info("x_lp_backorder_v=" + F2.getRecsIn()) ;
	}
    ...
    class Key implements Comparable {
        String      part_no ;
        String      location ;
        String      backorder_type ;

        public boolean equals(Object o) {
            Key k = (LpBackorder.Key) o ;
            return (k.part_no.equals(part_no) && k.location.equals(location) && k.backorder_type.equals(backorder_type) );
        }
        public int hashCode() {
            return part_no.hashCode() + location.hashCode() + backorder_type.hashCode() ;
        }
        public int compareTo(Object o) {
            Key theKey ;
            theKey = (Key) o ;
            return (theKey.part_no + theKey.location + theKey.backorder_type).compareTo(part_no + location + backorder_type) ;
        }
        public String toString() {
          return "part_no=" + part_no + " location=" + location + " backorder_type=" + backorder_type  ;
        }
    }
    Key key ;
    Key keyToCompare;
    ...
    LpBackorder(ResultSet r) throws SQLException {
        try {
        key = new Key() ;
        key.part_no = r.getString("PART") ;
        key.location = r.getString("LOCATION") ;
        key.backorder_type = r.getString("backorder_type") ;
...
//	}
	...
	public void writeFlatFile(String action)
	{
		long flatStart = System.currentTimeMillis();
			...
			key.backorder_type + "\", " + body.quantity 
		       	 + ", \"" + action + "\", " + batchNum);
