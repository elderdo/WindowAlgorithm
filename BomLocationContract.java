...
/*   $Author:   zf297a  $
   $Revision:   1.1  $
       $Date:   10 Apr 2009 14:36:12  $
   $Workfile:   BomLocationContract.java  $
        $Log:   I:\Program Files\Merant\vm\win32\bin\pds\archives\SDS-AMD\Components-ClientServer\WindowAlgorithm\BomLocationContract.java.-arc  $
/*
/*   Rev 1.1   10 Apr 2009 14:36:12   zf297a
/*Fixed formatting of date by the convertDate function: made it dd-MMM-yyyy - four characters for the year.
/*
/*   Rev 1.0   10 Apr 2009 13:42:52   zf297a
/*Initial revision.
...
public class BomLocationContract implements Rec {
    ...
    static Logger logger = Logger.getLogger(BomLocationContract.class.getName());
    ...
    private static TableSnapshot F1 = null ; // v_bom_location_contract (spo)
    private static TableSnapshot F2 = null ; // x_bom_location_contract_v (amd)

    // loads the parameter from this class's properties file if they exist
    static private void loadParams() {
	try {

		java.util.Properties p        = new AppProperties(BomLocationContract.class.getName()).getProperties() ;
		...
		flatFileName = String.valueOf( p.getProperty("dumpFile", "BomLocationContract.csv"));
	...
	if (useTestBatch) {
	    ...
	    batchNum = SpoBatch.createBatch("X_IMP_BOM_LOCATION_CONTRACT") ;			
	}
...
// keeps track of how long it took to read the 2 tables and form the arrays
	    ...
	    F1 = new TableSnapshot(bufSize, new BomLocationContractFactory("SELECT * FROM " + F1InstanceSQL + 
	    "V_BOM_LOCATION_CONTRACT order by bom, location, contract, contract_type, customer_location, begin_date", spo, prefetchSize)) ;
            F2 = new TableSnapshot(ageBufSize, new BomLocationContractFactory("SELECT * FROM " + F2InstanceSQL + 
	    "x_bom_location_contract_v order by bom, location, contract, contract_type, customer_location, begin_date" , amd, prefetchSize)) ;
	...
	if (F1 != null) {
	   	System.out.println("v_bom_location_contract=" + F1.getRecsIn()) ;
        	logger.info("v_bom_location_contract=" + F1.getRecsIn()) ;
	}
	if (F2 != null) {
	        System.out.println("x_bom_location_contract_v=" + F2.getRecsIn()) ;
        	logger.info("x_bom_location_contract_v=" + F2.getRecsIn()) ;
	}
    ...
    class Key implements Comparable {
        ...
        public boolean equals(Object o) {
            Key k = (BomLocationContract.Key) o ;
            return (k.bom.equals(bom)
            	&& k.location.equals(location)
            	&& k.contract.equals(contract)
            	&& k.contract_type.equals(contract_type)
            	&& k.customer_location.equals(customer_location)
            	&& k.begin_date.equals(begin_date));
        }
        public int hashCode() {
            return bom.hashCode() 
            	+ location.hashCode()  
            	+ contract.hashCode()  
            	+ contract_type.hashCode()  
            	+ customer_location.hashCode()  
            	+ begin_date.hashCode()  ;
        }
        public int compareTo(Object o) {
            Key theKey ;
            theKey = (Key) o ;
            return (theKey.bom + theKey.location + theKey.contract 
			    + theKey.contract_type
			    + theKey.customer_location + theKey.begin_date).compareTo(bom
				    + location + contract + contract_type + customer_location
				    + begin_date) ;
        }
	...
	java.sql.Date end_date ;
        ...
        public boolean equals(Object o) {
            ...
            boolean result = true ;
	    try {
		    result = result && b.end_date.equals(end_date) ;
		    showDiff(result, "b.end_date", b.end_date + "", end_date + "") ;
    ...
    BomLocationContract(ResultSet r) throws SQLException {
        try {
        key = new Key() ;
        key.bom = r.getString("BOM") ;
        key.location = r.getString("LOCATION") ;
        key.contract = r.getString("CONTRACT") ;
        key.contract_type = r.getString("CONTRACT_TYPE") ;
        key.customer_location = r.getString("CUSTOMER_LOCATION") ;
        key.begin_date = r.getDate("BEGIN_DATE");
