...
/*   $Author:   zf297a  $
   $Revision:   1.3  $
       $Date:   02 Dec 2008 11:34:24  $
   $Workfile:   LpInTransit.java  $
        $Log:   I:\Program Files\Merant\vm\win32\bin\pds\archives\SDS-AMD\Components-ClientServer\WindowAlgorithm\LpInTransit.java.-arc  $
/*
/*   Rev 1.3   02 Dec 2008 11:34:24   zf297a
...
public class LpInTransit implements Rec {
    ...
    static Logger logger = Logger.getLogger(LpInTransit.class.getName());
    ...
    private static TableSnapshot F1 = null ; // v_lp_in_transit (spo)
    private static TableSnapshot F2 = null ; // x_lp_in_transit_v (amd)

    // loads the parameter from this class's properties file if they exist
    static private void loadParams() {
	try {

		java.util.Properties p        = new AppProperties(LpInTransit.class.getName()).getProperties() ;
		...
		flatFileName = String.valueOf( p.getProperty("dumpFile", "LpInTransit.csv"));
	...
	if (useTestBatch) {
	    ...
	    batchNum = SpoBatch.createBatch("X_IMP_LP_IN_TRANSIT") ;			
	}
...
// keeps track of how long it took to read the 2 tables and form the arrays
	    ...
	    F1 = new TableSnapshot(bufSize, new LpInTransitFactory("SELECT * FROM " + F1InstanceSQL + 
	    "V_LP_IN_TRANSIT order by location, part, in_transit_type", spo, prefetchSize)) ;
            F2 = new TableSnapshot(ageBufSize, new LpInTransitFactory("SELECT * FROM " + F2InstanceSQL + 
	    "x_lp_in_transit_v order by location, part, in_transit_type" , amd, prefetchSize)) ;
	...
	if (F1 != null) {
	   	System.out.println("v_lp_in_transit=" + F1.getRecsIn()) ;
        	logger.info("v_lp_in_transit=" + F1.getRecsIn()) ;
	}
	if (F2 != null) {
	        System.out.println("x_lp_in_transit_v=" + F2.getRecsIn()) ;
        	logger.info("x_lp_in_transit_v=" + F2.getRecsIn()) ;
	}
    ...
    class Key implements Comparable {
        String      part_no ;
        String      location ;
        String      in_transit_type ;

        public boolean equals(Object o) {
            Key k = (LpInTransit.Key) o ;
            return (k.part_no.equals(part_no) && k.location.equals(location) && k.in_transit_type.equals(in_transit_type) );
        }
        public int hashCode() {
            return part_no.hashCode() + location.hashCode() + in_transit_type.hashCode() ;
        }
        public int compareTo(Object o) {
            Key theKey ;
            theKey = (Key) o ;
            return (theKey.part_no + theKey.location + theKey.in_transit_type).compareTo(part_no + location + in_transit_type) ;
        }
        public String toString() {
          return "part_no=" + part_no + " location=" + location + " in_transit_type=" + in_transit_type  ;
        }
    }
    Key key ;
    Key keyToCompare;
    ...
    LpInTransit(ResultSet r) throws SQLException {
        try {
        key = new Key() ;
        key.part_no = r.getString("PART") ;
        key.location = r.getString("LOCATION") ;
        key.in_transit_type = r.getString("IN_TRANSIT_TYPE") ;
...
//	}
	...
	public void writeFlatFile(String action)
	{
		long flatStart = System.currentTimeMillis();
			...
			key.in_transit_type + "\", " + body.quantity 
		       	 + ", \"" + action + "\", " + batchNum);
