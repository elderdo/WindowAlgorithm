...
/*   $Author:   zf297a  $
   $Revision:   1.2  $
       $Date:   25 Mar 2009 22:37:18  $
   $Workfile:   LpDemandForecast.java  $
        $Log:   I:\Program Files\Merant\vm\win32\bin\pds\archives\SDS-AMD\Components-ClientServer\WindowAlgorithm\LpDemandForecast.java.-arc  $
/*
/*   Rev 1.2   25 Mar 2009 22:37:18   zf297a
/*Fixed equals method - used compareTo for the BigDecimal quantity instead of ==
/*
/*   Rev 1.1   14 Nov 2008 09:41:06   zf297a
/*Changed the quantity variable to be BigDecimal and fixed the formating of the flat file.
/*
/*   Rev 1.0   10 Nov 2008 19:03:04   zf297a
/*Initial revision.


	*/

public class LpDemandForecast implements Rec {
    ...
    static Logger logger = Logger.getLogger(LpDemandForecast.class.getName());

    static FileOutputStream o1;
    static PrintStream p; // declare a print stream object  
    static long batchNum;
    static String s = "INSERT INTO " + F1Instance + ".X_lp_demand_v(TRANSACTION_ID, LOCATION, PART, CONTRACT, DEMAND_TYPE, DEMAND_DATE CUSTOMER_LOCATION, PRODUCT_SERIAL_NUMBER, QUANTITY," + 
    ...
    private static TableSnapshot F1 = null ; // v_lp_demand
    private static TableSnapshot F2 = null ; // x_lp_demand_v

    // loads the parameter from this class's properties file if they exist
    static private void loadParams() {
	try {

		java.util.Properties p        = new AppProperties(LpDemandForecast.class.getName()).getProperties() ;
		...
		flatFileName = String.valueOf( p.getProperty("dumpFile", "LpDemandForecast.csv"));
	...
	if (useTestBatch) {
	    ...
	    batchNum = SpoBatch.createBatch("X_IMP_LP_DEMAND_FORECAST") ;			
	}
...
// keeps track of how long it took to read the 2 tables and form the arrays
	    ...
	    F1 = new TableSnapshot(bufSize, new LpDemandForecastFactory("SELECT * FROM " + F1InstanceSQL + 
	    "V_LP_DEMAND_FORECAST order by location, part, period", spo, prefetchSize)) ;
            F2 = new TableSnapshot(ageBufSize, new LpDemandForecastFactory("SELECT * FROM " + F2InstanceSQL + 
	    "x_lp_demand_forecast_v where location is not null and part is not null and period is not null order by location, part, period" , amd, prefetchSize)) ;
    ...
    class Key implements Comparable {
        String      	location ;
        String      	part ;
        String      	demand_forecast_type ;
        java.sql.Date	period ;
      

        public boolean equals(Object o) {
            Key k = (LpDemandForecast.Key) o ;
            return (k.location.equals(location) && k.part.equals(part)
	    && k.demand_forecast_type.equals(demand_forecast_type) && k.period.equals(period));
        }
        public int hashCode() {
            return location.hashCode() + part.hashCode() + demand_forecast_type.hashCode() + period.hashCode() ;
        }
        public int compareTo(Object o) {
            Key theKey ;
            theKey = (Key) o ;
            return (theKey.location + theKey.part + theKey.demand_forecast_type + theKey.period).compareTo(location + part
			    + demand_forecast_type + period) ;
        }
    ...
    class Body {
        ...
        public boolean equals(Object o) {
            Body b = (Body) o ;
            boolean result ;
            result = b.quantity.compareTo(quantity) == 0 ;
    ...
    LpDemandForecast(ResultSet r) throws SQLException {
        try {
        key = new Key() ;
        key.location = r.getString("LOCATION") ;
        key.part = r.getString("PART") ;
        key.demand_forecast_type = r.getString("DEMAND_FORECAST_TYPE") ;
        key.period = r.getDate("PERIOD") ;
       
        body = new Body() ;
        body.quantity = r.getBigDecimal("QUANTITY") ;
        }
	...
	public void writeFlatFile(String action)
	{
		long flatStart = System.currentTimeMillis();
			p.println("\"" + key.location 
				+ "\", \"" + key.part 
				+ "\", \"" + key.demand_forecast_type 
				+ "\", " + convertDate(key.period) 
				+ ", " + body.quantity 
		       	 	+ ", \"" + action + "\", " + batchNum);
