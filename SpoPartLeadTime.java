...
/*   $Author:   zf297a  $
   $Revision:   1.2  $
       $Date:   02 Dec 2008 11:53:54  $
   $Workfile:   SpoPartLeadTime.java  $
        $Log:   I:\Program Files\Merant\vm\win32\bin\pds\archives\SDS-AMD\Components-ClientServer\WindowAlgorithm\SpoPartLeadTime.java.-arc  $
/*
/*   Rev 1.2   02 Dec 2008 11:53:54   zf297a
/*Fixed F1 (spo view) and F2 (amd x_ view) comments.  Fixed literals for updateCounts.  Removed s variable that was not being used.
/*
/*   Rev 1.1   12 Sep 2008 18:37:52   zf297a
/*Fixed action_code for the flat file.
/*
/*   Rev 1.0   12 Sep 2008 14:00:46   zf297a
/*Initial revision.

	*/

public class SpoPartLeadTime implements Rec {
    ...
    static Logger logger = Logger.getLogger(SpoPartLeadTime.class.getName());
    ...
    private static TableSnapshot F1 = null ; // v_part_lead_time (spo)
    private static TableSnapshot F2 = null ; // x_part_lead_time_v (amd)

    // loads the parameter from this class's properties file if they exist
    static private void loadParams() {
	try {

		java.util.Properties p        = new AppProperties(SpoPartLeadTime.class.getName()).getProperties() ;
		...
		flatFileName = String.valueOf( p.getProperty("dumpFile", "SpoPartLeadTime.csv"));
	...
	if (useTestBatch) {
	    ...
	    batchNum = SpoBatch.createBatch("X_IMP_PART_LEAD_TIME") ;			
	}
...
// keeps track of how long it took to read the 2 tables and form the arrays
	    ...
	    F1 = new TableSnapshot(bufSize, new SpoPartLeadTimeFactory("SELECT * FROM " + F1InstanceSQL + 
	    "V_PART_LEAD_TIME order by part, lead_time_type", spo, prefetchSize)) ;
            F2 = new TableSnapshot(ageBufSize, new SpoPartLeadTimeFactory("SELECT * FROM " + F2InstanceSQL + 
	    "x_part_lead_time_v order by part, lead_time_type" , amd, prefetchSize)) ;
	...
	if (F1 != null) {
	   	System.out.println("v_part_lead_time=" + F1.getRecsIn()) ;
        	logger.info("v_part_lead_time=" + F1.getRecsIn()) ;
	}
	if (F2 != null) {
	        System.out.println("x_part_lead_time_v=" + F2.getRecsIn()) ;
        	logger.info("x_part_lead_time_v=" + F2.getRecsIn()) ;
	}
    ...
    class Key implements Comparable {
        ...
        public boolean equals(Object o) {
            Key k = (SpoPartLeadTime.Key) o ;
            return (k.part.equals(part) && k.lead_time_type.equals(lead_time_type) );
        }
        public int hashCode() {
            return part.hashCode() + lead_time_type.hashCode()  ;
        }
        public int compareTo(Object o) {
            Key theKey ;
            theKey = (Key) o ;
            return (theKey.part + theKey.lead_time_type).compareTo(part + lead_time_type) ;
	}

        public String toString() {
          return "part=" + part + " lead_time_type=" + lead_time_type   ;
        }
    }
    Key key ;
    Key keyToCompare;
    class Body {
	 ...
	 + " variance=" + variance ;
        ...
        public boolean equals(Object o) {
            ...
            result = result && b.variance == variance ;
            showDiff(result, "variance", b.variance + "", variance + "") ;
            return result ;
        }
    }
    Body body ;
    ...
    SpoPartLeadTime(ResultSet r) throws SQLException {
        try {
        key = new Key() ;
        key.part = r.getString("PART") ;
        key.lead_time_type = r.getString("LEAD_TIME_TYPE") ;

        body = new Body() ;
        body.quantity = r.getDouble("QUANTITY") ;
        body.variance = r.getInt("VARIANCE") ;
        }
